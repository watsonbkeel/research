...
export async function buildSectionEvidenceBundle(input: { projectId: string; documentId: string; sectionId: string; mode?: "exploratory" | "formal" }) {
  ...
  const mode = input.mode ?? (document.mode === "empirical" ? "formal" : "exploratory"); const excerpts = await listEvidenceExcerpts({ projectId: input.projectId }); const works = new Map(workspace.works.map((work) => [work.id, work]));
