...
import { referencesFor, renderCitationTokens, renderVersionCitations, renderVersionSectionContent } from "./citation-service";
...
export function exportProjectDocumentMarkdown(project: ProjectRecord, document: ProjectDocument, workspace?: WorkspaceData, audit?: CitationAuditReport) {
  ...
  const formalRendered = document.versionSnapshot ? renderVersionCitations(document.versionSnapshot) : undefined;
  for (const chapter of document.manuscript.chapters) {
    lines.push(`## ${chapter.number}. ${chapter.title}`, "");
    for (const section of chapter.sections) { const rendered = formalRendered && document.versionSnapshot ? renderVersionSectionContent(document.versionSnapshot, section.id, section.content, formalRendered) : workspace ? renderCitationTokens(section.content, workspace.works, cited, style).content : section.content.replace(/\[\[CITE:[^\]]+\]\]/g, ""); lines.push(rendered.trim() || "_[Section not drafted]_", ""); }
  }
  if (formalRendered || workspace) lines.push("## References", "", ...(formalRendered?.bibliography ?? referencesFor(workspace!.works, cited, style)).map((item) => `- ${item.text}`), "");
...
export async function exportProjectDocumentDocx(project: ProjectRecord, document: ProjectDocument, workspace?: WorkspaceData, audit?: CitationAuditReport) {
  ...
  for (const chapter of document.manuscript.chapters) {
    ...
    for (const section of chapter.sections) {
      ...
      const renderedSection = formalRendered && document.versionSnapshot ? renderVersionSectionContent(document.versionSnapshot, section.id, section.content, formalRendered) : workspace ? renderCitationTokens(section.content, workspace.works, cited, style).content : section.content.replace(/\[\[CITE:[^\]]+\]\]/g, "");
  ...
  if (formalRendered || workspace) { body.push(new Paragraph({ text: "References", heading: HeadingLevel.HEADING_1 })); for (const item of formalRendered?.bibliography ?? referencesFor(workspace!.works, cited, style)) body.push(new Paragraph({ text: item.text, spacing: { after: 120 } })); }
