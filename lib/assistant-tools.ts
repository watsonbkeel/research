...
import { listQuarantinedDrafts } from "./evidence-store";
...
export async function getCurrentDocument(projectId: string, documentId?: string) { const documents = (await import("./project-documents")).listProjectDocuments(projectId); const document = documentId ? documents.find((item) => item.id === documentId) : documents[0]; const coverage = document ? latestClaimCoverage(projectId, document.id) : undefined; return document ? { id: document.id, title: document.title, mode: document.mode, researchMode: document.researchMode, evidenceMode: document.evidenceMode, currentVersionId: document.currentVersionId, currentVersionNumber: document.currentVersionNumber, status: document.status, quarantinedDrafts: listQuarantinedDrafts(projectId, document.id), coverage: coverage ? { status: coverage.status, blockers: coverage.blockers.length, warnings: coverage.warnings.length } : undefined, chapters: document.manuscript.chapters.map((chapter) => ({ id: chapter.id, number: chapter.number, title: chapter.title, sections: chapter.sections.map((section) => ({ id: section.id, number: section.number, title: section.title, citationCount: section.citationIds.length, unsupportedCount: section.unsupportedStatements.length, words: section.content.split(/\s+/).filter(Boolean).length })) })) } : undefined; }
