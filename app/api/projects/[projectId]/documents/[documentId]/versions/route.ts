...
import { listProjectDocumentVersions, restoreProjectDocumentVersion } from "@/lib/project-documents";
export const runtime = "nodejs";
type Context = { params: Promise<{ projectId: string; documentId: string }> };
export async function GET(request: Request, context: Context) { const { projectId, documentId } = await context.params; try { return NextResponse.json({ versions: listProjectDocumentVersions(projectId, documentId, new URL(request.url).searchParams.get("sectionId") ?? undefined) }); } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "版本加载失败。" }, { status: 404 }); } }
