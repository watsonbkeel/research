...
export async function runCitationAudit(input: { projectId: string; documentId: string; versionId?: string; formal?: boolean }) {
  const document = getProjectDocument(input.projectId, input.documentId); if (!document) throw new Error("文档不存在。"); const workspace = await readWorkspace(input.projectId); const excerpts = await listEvidenceExcerpts({ projectId: input.projectId });
  ...
  for (const section of document.manuscript.chapters.flatMap((chapter) => chapter.sections)) {
    ...
    for (const id of section.citationIds) { const work = works.get(id); if (!work) blockers.push({ code: "missing-work", severity: "blocker", message: `章节引用了不存在的 Work ${id}。`, sectionId: section.id, workId: id }); else if (work.bibliographicStatus !== "verified") blockers.push({ code: "work-not-verified", severity: "blocker", message: `Work ${id} 尚未完成书目核验。`, sectionId: section.id, workId: id }); else if (work.retractionStatus === "retracted") blockers.push({ code: "retracted-work", severity: "blocker", message: `Work ${id} 已撤稿。`, sectionId: section.id, workId: id }); else if (work.retractionStatus === "corrected") warnings.push({ code: "corrected-work", severity: "warning", message: `Work ${id} 存在更正记录，正式使用前必须阅读更正内容。`, sectionId: section.id, workId: id });
      if (!tokenIds.has(id)) (input.formal ? blockers : warnings).push({ code: "reference-not-in-body", severity: input.formal ? "blocker" : "warning", message: `Work ${id} 出现在章节参考文献记录中，但正文没有对应 citation token。`, sectionId: section.id, workId: id });
    }
    for (const id of tokenIds) if (!section.citationIds.includes(id)) (input.formal ? blockers : warnings).push({ code: "body-citation-not-in-reference-record", severity: input.formal ? "blocker" : "warning", message: `正文 citation token ${id} 没有同步到章节 citationIds。`, sectionId: section.id, workId: id });
    for (const evidenceId of section.evidenceExcerptIds) {
      ...
      if (input.formal && effectiveVerificationStatus(excerpt) !== "human_verified") blockers.push({ code: "evidence-not-human-verified", severity: "blocker", message: `正式模式不能使用未由研究者确认的 EvidenceExcerpt ${evidenceId}。`, sectionId: section.id, evidenceExcerptId: evidenceId });
    ...
    for (const unsupported of section.unsupportedStatements) (input.formal ? blockers : warnings).push({ code: "unsupported-statement", severity: input.formal ? "blocker" : "warning", message: `章节仍有未支持论断：${unsupported.statement}`, sectionId: section.id });
    for (const gap of section.evidenceGaps ?? []) (input.formal ? blockers : warnings).push({ code: "evidence-gap", severity: input.formal ? "blocker" : "warning", message: `章节存在证据缺口：${gap}`, sectionId: section.id });
  ...
  const report: CitationAuditReport = { id: `citation-audit-${randomUUID()}`, projectId: input.projectId, documentId: input.documentId, versionId: input.versionId ?? document.manuscript.version, status: blockers.length ? "blocked" : warnings.length ? "passed_with_warnings" : "passed", blockers, warnings, checkedAt: new Date().toISOString(), checkerVersion: "citation-audit-v2" };
  return saveCitationAudit(report);
}
