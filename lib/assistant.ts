...
const { DatabaseSync } = require("node:sqlite") as { DatabaseSync: new (file: string) => Db };
let db: Db | undefined;
let dbPath = "";
const now = () => new Date().toISOString();
const id = () => randomUUID();

export const conversationInputSchema = z.object({ title: z.string().trim().min(1).max(300).optional(), projectId: z.string().max(120).optional(), topicBatchId: z.string().max(120).optional(), metadata: z.record(z.unknown()).optional() }).strict();
export const messageInputSchema = z.object({ role: z.enum(["user", "assistant", "system", "tool"]), content: z.string().min(1).max(100_000), metadata: z.record(z.unknown()).optional() }).strict();
export const conversationMessageInputSchema = z.object({ content: z.string().min(1).max(100_000), profileId: z.string().min(1).max(120).optional() }).strict();
export const jobInputSchema = z.object({ conversationId: z.string().min(1).optional(), prompt: z.string().min(1).max(100_000), kind: z.string().trim().min(1).max(80).default("research"), input: z.record(z.unknown()).optional() }).strict();
export const actionInputSchema = z.object({
  action: z.enum(["pause", "resume", "cancel", "retry", "confirm-proposal", "approve-evidence", "apply-revision"]),
  revisionId: z.string().optional(),
  evidenceExcerptId: z.string().optional(),
  reviewer: z.string().trim().min(1).max(200).optional(),
  reviewedAt: z.string().datetime().optional(),
}).strict();
export const artifactInputSchema = z.object({ jobId: z.string().min(1), type: z.string().min(1).max(80), title: z.string().max(300).optional(), content: z.unknown(), metadata: z.record(z.unknown()).optional() }).strict();
export const candidateInputSchema = z.object({ jobId: z.string().min(1), title: z.string().min(1).max(500), url: z.string().url().max(4000).optional(), source: z.string().max(300).optional(), abstract: z.string().max(50_000).optional(), metadata: z.record(z.unknown()).optional() }).strict();
...
export function getConversation(conversationId: string): Conversation | undefined { const row = database().prepare("SELECT id,title,metadata,project_id AS projectId,topic_batch_id AS topicBatchId,created_at AS createdAt,updated_at AS updatedAt FROM assistant_conversations WHERE id=?").get(conversationId); return row ? conversationRow(row) : undefined; }
...
export function createProposalGenerationJob(sourceJobId: string, prompt?: string): ResearchJob {
  ...
  if (!listArtifacts(sourceJobId).some((artifact) => artifact.type === "feasibility-report")) throw new Error("该任务尚未生成可行性报告，暂时不能生成 Proposal。");
  return createResearchJob({
    conversationId: source.conversationId,
    prompt: prompt?.trim() || source.prompt,
    kind: "proposal-generation",
    input: {
      feasibilityJobId: source.id,
      stage: "proposal-outline",
      userDirected: true,
...
export function updateResearchJob(jobId: string, patch: { input?: Record<string, unknown>; error?: string | null }) {
  ...
  const nextInput = patch.input ? { ...job.input, ...patch.input } : job.input;
  database().prepare("UPDATE assistant_jobs SET input=?,error=?,updated_at=? WHERE id=?").run(json(nextInput), patch.error === undefined ? job.error ?? null : patch.error, now(), jobId);
  return getResearchJob(jobId)!;
}
export function addArtifact(input: z.input<typeof artifactInputSchema>) { const p = artifactInputSchema.parse(input), artifact = { id: id(), jobId: p.jobId, type: p.type, title: p.title, content: p.content, metadata: p.metadata ?? {}, createdAt: now() }; const job = getResearchJob(p.jobId); if (!job) throw new Error("Job not found"); database().prepare("INSERT INTO assistant_artifacts (id,job_id,project_id,document_id,type,title,content,metadata,created_at) VALUES (?,?,?,?,?,?,?,?,?)").run(artifact.id, artifact.jobId, job.projectId ?? null, job.documentId ?? null, artifact.type, artifact.title ?? null, json(artifact.content), json(artifact.metadata), artifact.createdAt); addJobEvent(p.jobId, "artifact", { artifactId: artifact.id, type: artifact.type }); return artifact; }
export function listArtifacts(jobId: string) { return database().prepare("SELECT id,job_id AS jobId,type,title,content,metadata,created_at AS createdAt FROM assistant_artifacts WHERE job_id=? ORDER BY created_at").all(jobId).map((r: any) => ({ ...r, content: parse(r.content, r.content), metadata: parse(r.metadata, {}) })); }
export function addCandidate(input: z.input<typeof candidateInputSchema>) { const p = candidateInputSchema.parse(input), candidate = { id: id(), ...p, metadata: p.metadata ?? {}, createdAt: now() }; const job = getResearchJob(p.jobId); if (!job) throw new Error("Job not found"); database().prepare("INSERT INTO assistant_candidates (id,job_id,project_id,topic_candidate_id,title,url,source,abstract,metadata,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)").run(candidate.id, candidate.jobId, job.projectId ?? null, job.candidateId ?? null, candidate.title, candidate.url ?? null, candidate.source ?? null, candidate.abstract ?? null, json(candidate.metadata), candidate.createdAt); return candidate; }
export function listCandidates(jobId: string) { return database().prepare("SELECT id,job_id AS jobId,title,url,source,abstract,metadata,created_at AS createdAt FROM assistant_candidates WHERE job_id=? ORDER BY created_at").all(jobId).map((r: any) => ({ ...r, metadata: parse(r.metadata, {}) })); }
