...
describe("project evidence excerpt route locator handling", () => {
  it("accepts typed non-page locators, rejects untyped locators, and patches legacy records", async () => {
    ...
    updateWorkVerification(project.id, work.id, { id: "verification-route", projectId: project.id, workId: work.id, provider: "manual", inputIdentifier: work.id, checkedAt: "2026-08-20T00:00:00.000Z", matchedFields: { doi: false, title: true, authors: true, year: true, venue: true }, result: "verified", retractionStatus: "clear" });
    ...
    const typedExcerpt = (await typed.json()).excerpt as { id: string };
    expect(typedExcerpt).toMatchObject({ locatorType: "chapter", locator: "Chapter 3" });
    const patched = await PATCH(new Request(`http://localhost/api/projects/${project.id}/evidence-excerpts`, { method: "PATCH", body: JSON.stringify({ id: typedExcerpt.id, locatorType: "table", locator: "Table 4" }), headers: { "content-type": "application/json" } }), { params: Promise.resolve({ projectId: project.id }) });
    expect(patched.status).toBe(200);
    expect((await patched.json()).excerpt).toMatchObject({ locatorType: "table", locator: "Table 4" });
    const invalid = await POST(new Request(`http://localhost/api/projects/${project.id}/evidence-excerpts`, { method: "POST", body: JSON.stringify({ workId: work.id, locatorType: "unknown", locator: "Methods", paraphrase: "A note" }), headers: { "content-type": "application/json" } }), { params: Promise.resolve({ projectId: project.id }) });
