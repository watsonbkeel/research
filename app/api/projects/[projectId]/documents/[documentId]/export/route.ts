...
export async function GET(request: Request, context: { params: Promise<{ projectId: string; documentId: string }> }) {
  ...
  const format = new URL(request.url).searchParams.get("format") ?? "docx"; const formal = new URL(request.url).searchParams.get("formal") === "1"; const slug = safeFileSlug(document.title); const workspace = await readWorkspace(projectId); const audit = await runCitationAudit({ projectId, documentId, formal });
  if (formal && audit.blockers.length) return NextResponse.json({ error: "正式导出被引用审查阻断。", audit }, { status: 409 });
  const draftAudit = formal ? undefined : audit;
