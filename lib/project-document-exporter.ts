...
export async function exportProjectDocumentDocx(project: ProjectRecord, document: ProjectDocument, workspace?: WorkspaceData, audit?: CitationAuditReport) {
  ...
  for (const chapter of document.manuscript.chapters) {
    ...
    for (const section of chapter.sections) {
      ...
      const clusters = document.versionSnapshot?.citationClusters?.filter((item) => item.sectionId === section.id).sort((left, right) => (left.documentOrder ?? Number.MAX_SAFE_INTEGER) - (right.documentOrder ?? Number.MAX_SAFE_INTEGER) || left.position - right.position) ?? []; let clusterIndex = 0; const renderedSection = formalRendered ? section.content.replace(/\[\[CITE:[^\]]+\]\]/g, () => formalRendered.citations.get(clusters[clusterIndex++]?.id ?? "") ?? "") : workspace ? renderCitationTokens(section.content, workspace.works, cited, style).content : section.content.replace(/\[\[CITE:[^\]]+\]\]/g, "");
