...
import { listFullTextAssets, searchLocalFullText } from "../lib/full-text";
...
async function processAssistantTask(job: ResearchJob, owner: string, leaseMs: number) {
  ...
  if (intent === "section_revision") {
    const documentId = String(job.documentId ?? job.input.documentId ?? ""); const sectionId = String(job.input.sectionId ?? ""); const workflowId = text(job.input.workflowRunId);
    const advance = (state: import("../lib/types").AssistantWorkflowRun["state"], tool: string, status: "completed" | "blocked" = "completed", outputSummary?: string) => { if (workflowId) advanceAssistantWorkflow(projectId, workflowId, state, { tool, inputSummary: `worker job ${job.id}`, outputSummary, status }); };
    const document = getProjectDocument(projectId, documentId); const section = document?.manuscript.chapters.flatMap((chapter) => chapter.sections).find((item) => item.id === sectionId); if (!document || !section) throw new Error("章节修订工作流的 document 或 section 不存在。");
    advance("analyzing_section", "get_document_version", "completed", document.currentVersionId);
    const coverage = await compileClaimCoverage({ projectId, documentId, versionId: document.currentVersionId }); advance("compiling_claim_coverage", "compile_claim_coverage", "completed", `${coverage.status}: ${coverage.blockers.length} blockers`);
    const preAudit = await runCitationAudit({ projectId, documentId, versionId: document.currentVersionId, formal: document.evidenceMode === "formal" }); advance("auditing", "run_citation_audit", "completed", `${preAudit.status}: ${preAudit.blockers.length} blockers`);
    const existing = await buildSectionEvidenceBundle({ projectId, documentId, sectionId }); advance("matching_existing_evidence", "match_existing_claim_evidence", "completed", `${existing.bundle.claims.length} claims; ${existing.bundle.unresolvedClaims.length} unresolved`);
    if (existing.bundle.unresolvedClaims.length) {
      const search = await runWithHeartbeat(job, owner, (signal) => searchAcademicMetadata(`${section.title} ${section.content.slice(0, 1500)}`, { perProvider: 8, signal }), leaseMs) as Awaited<ReturnType<typeof searchAcademicMetadata>>;
      const savedCandidates = search.candidates.map((candidate) => saveCandidateRecord({ id: stableCandidateId(projectId, candidate.provider, candidate.sourceId), projectId, provider: candidate.provider, providerRecordId: candidate.sourceId, title: candidate.title, authors: candidate.authors, year: candidate.year, venue: candidate.venue, doi: candidate.doi, url: candidate.url, abstract: candidate.abstract })); advance("searching_candidates", "search_literature", "completed", `${savedCandidates.length} candidates persisted`);
      const verified = []; for (const candidate of savedCandidates.filter((item) => item.doi).slice(0, 3)) verified.push(await verifyCandidateBibliography({ projectId, candidateId: candidate.id })); advance("verifying_metadata", "verify_bibliographic_record", "completed", `${verified.filter((item) => item.event.result === "verified").length} verified`);
      if (!listFullTextAssets(projectId).length) { advance("awaiting_full_text", "search_local_full_text", "blocked", "No project-scoped full text is available; upload or register permitted full text to continue."); addMessage(job.conversationId ?? "", { role: "assistant", content: `已完成 Coverage、CitationAudit、候选检索和书目核验。当前没有可用全文，工作流停在 awaiting_full_text；候选元数据没有被当作证据或正式引用。`, metadata: { jobId: job.id, workflowId, candidateCount: savedCandidates.length } }); transitionJob(job.id, "waiting-user"); return; }
      const workspace = await readWorkspace(projectId); const suggested = [];
      for (const unresolved of existing.bundle.unresolvedClaims) { const claim = workspace.claims.find((item) => item.id === unresolved.claimId); if (!claim) continue; for (const hit of searchLocalFullText(projectId, claim.text.slice(0, 120)).slice(0, 3)) { const work = workspace.works.find((item) => item.id === hit.workId); if (!work || work.bibliographicStatus !== "verified") continue; try { suggested.push(await createEvidenceExcerpt({ id: `excerpt-${workflowId || job.id}-${claim.id}-${hit.assetId}-${hit.page}`.slice(0, 200), workId: work.id, fullTextAssetId: hit.assetId, page: hit.page, locator: `PDF page ${hit.page}`, claimId: claim.id, paraphrase: hit.text.slice(0, 2000), supportDirection: "supporting", verificationStatus: "ai_suggested", externalModelUsePermission: "prohibited", exportPermission: "unknown" }, projectId)); } catch { /* idempotent retries retain the existing suggestion */ } } }
      const pendingHuman = (await listEvidenceExcerpts({ projectId })).filter((item) => section.claimIds.includes(item.claimId ?? "") && item.verificationStatus === "ai_suggested");
      if (suggested.length || pendingHuman.length) { advance("suggesting_excerpts", "suggest_evidence_excerpts", "completed", `${pendingHuman.length} AI suggestions`); advance("awaiting_human_verification", "create_ai_suggested_excerpt", "blocked", "Researcher verification is required before drafting."); addMessage(job.conversationId ?? "", { role: "assistant", content: `已生成 ${pendingHuman.length} 条 ai_suggested EvidenceExcerpt。它们不会自动变为 human_verified；请核对题名、DOI、原文、页码、Claim 和证据方向后批准或拒绝。`, metadata: { jobId: job.id, workflowId, evidenceExcerptIds: pendingHuman.map((item) => item.id) } }); transitionJob(job.id, "waiting-user"); return; }
      advance("blocked", "suggest_evidence_excerpts", "blocked", "Full text exists but no claim-level passages matched safely."); transitionJob(job.id, "waiting-user"); return;
    }
